...
int val1;
bool state = 0; // true = play , false = pause
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    val1 = digitalRead(15); // Switch 1
      Serial.println(num);
     
  }
      
  send7Seg_MultiDigit(font[num/10],1); //แยกหลักสิบ
  send7Seg_MultiDigit(font[num%10],2); //แยกหลักหน่วย
  }
